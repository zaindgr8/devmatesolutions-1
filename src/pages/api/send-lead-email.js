...
export default async function handler(req, res) {
  ...
  const isDemo = /demo|ai lead management/i.test(sourceLabel);
...
<head>
  <meta charset="UTF-8" />
  <title>${isDemo ? 'New Demo Request' : 'New Lead'} — DevMate Solutions</title>
</head>
<body style="margin:0;padding:0;background:#f4f4f5;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="background:#0d0d0d;padding:28px 40px;">
              <h1 style="margin:0;color:#ffffff;font-size:18px;font-weight:800;">
                ${isDemo ? '🎯 New Demo Booking: AI Lead Management' : '📬 New Inquiry Dropped — DevMate Solutions'}
              </h1>
          ...
          <tr>
            <td style="background:#fef2f2;border-left:4px solid #c0392b;padding:16px 40px;">
              <p style="margin:0;font-size:14px;font-weight:700;color:#c0392b;">
                ${isDemo ? 'ACTION REQUIRED — New AI Lead Management demo inquiry received' : 'ACTION REQUIRED — New lead inquiry received'}
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:20px 40px 0;">
              <span style="display:inline-block;background:${isDemo ? '#fef2f2' : '#f0fdf4'};border:1px solid ${isDemo ? '#fecaca' : '#bbf7d0'};color:${isDemo ? '#bd2120' : '#16a34a'};font-size:12px;font-weight:700;padding:4px 12px;border-radius:100px;letter-spacing:0.5px;">
                Use Case / Source: ${sourceLabel}
              </span>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:24px 40px 32px;">
              <h3 style="margin:0 0 16px;font-size:16px;font-weight:800;color:#0d0d0d;">Inquiry Details</h3>
              <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #f0f0f0;border-radius:8px;overflow:hidden;">
                ...
                <tr style="background:#f9fafb;">
                  <td style="padding:12px 16px;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#9ca3af;border-top:1px solid #f0f0f0;">Contact Number</td>
                ...
                <tr>
                  <td style="padding:12px 16px;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#9ca3af;border-top:1px solid #f0f0f0;">Inquiry Type</td>
                  <td style="padding:12px 16px;font-size:14px;font-weight:600;color:#0d0d0d;border-top:1px solid #f0f0f0;">
                    ${isDemo ? 'AI Lead Management System Demo' : 'General Service Inquiry'}
                  </td>
                </tr>
                ${query ? `
                <tr style="background:#f9fafb;">
                  <td style="padding:12px 16px;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#9ca3af;border-top:1px solid #f0f0f0;vertical-align:top;">Query / Requirements</td>
                  <td style="padding:12px 16px;font-size:14px;color:#334155;border-top:1px solid #f0f0f0;line-height:1.5;">${query}</td>
                </tr>
                ` : ''}
              </table>
            </td>
          ...
          <tr>
            <td style="padding:0 40px 32px;text-align:center;">
              ${email ? `
              <a href="mailto:${email}"
                style="display:inline-block;background:#bd2120;color:#ffffff;font-size:14px;font-weight:700;padding:12px 28px;border-radius:8px;text-decoration:none;margin-right:10px;">
                ✉️ Reply via Email
              </a>
              ` : ''}
              <a href="https://wa.me/${(country || '').replace('+', '')}${contact.replace(/\s+/g, '')}"
                style="display:inline-block;background:#0d0d0d;color:#ffffff;font-size:14px;font-weight:700;padding:12px 24px;border-radius:8px;text-decoration:none;">
                💬 Open WhatsApp
              </a>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:0 40px 28px;text-align:center;">
              <p style="margin:0;font-size:12px;color:#d1d5db;">DevMate Solutions Lead Intake System</p>
            </td>
          </tr>
  ...
  try {
    ...
    const subjectPrefix = isDemo ? '🎯 New Demo Booking (AI Lead System)' : '📞 New Lead';
    let mgmtResult = await resend.emails.send({
      from: fromAddress,
      to: ['contact@devmatesolutions.com'],
      subject: `${subjectPrefix}: ${name || contact} — ${sourceLabel}`,
      html: notificationHtml,
      replyTo: email || undefined,
    });

    if (mgmtResult.error) {
      console.warn('Management email send error:', mgmtResult.error);
...
<head>
  ...
  <title>${isDemo ? 'Your Demo is Booked – DevMate Solutions' : 'Welcome to DevMate Solutions'}</title>
</head>
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 6px 30px rgba(0,0,0,0.06);max-width:600px;width:100%;">
          ...
          <tr>
            <td style="padding:36px 36px 20px;">
              <p style="margin:0 0 6px;font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:1.5px;color:#bd2120;">
                ${isDemo ? 'Demo Confirmed' : 'Request Received'}
              </p>
              <h2 style="margin:0 0 16px;font-size:24px;font-weight:900;color:#0f172a;line-height:1.25;">
                Hi ${userFirstName}, ${isDemo ? "your AI Demo is booked!" : "we've received your request!"} 🚀
              </h2>
              <p style="margin:0;font-size:15px;color:#475569;line-height:1.65;">
                ${isDemo
                  ? `Thank you for booking a demo for our <strong>AI Lead Management System</strong>. Our solutions team is reviewing your details and will connect with you to deliver your tailored live walkthrough.`
              ...
              <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:14px 18px;margin-top:16px;">
                <p style="margin:0 0 4px;font-size:11px;font-weight:700;text-transform:uppercase;color:#94a3b8;letter-spacing:0.5px;">Your Submitted Query / Requirements:</p>
                <p style="margin:0;font-size:13.5px;color:#334155;line-height:1.5;">${query}</p>
              </div>
              ` : ''}
            </td>
          </tr>

          <!-- PRIMARY FOCUS: AI Lead Management System Demo -->
          <tr>
                  ...
                  <tr>
                    <td>
                      <span style="display:inline-block;background:#bd2120;color:#ffffff;font-size:11px;font-weight:800;padding:3px 10px;border-radius:100px;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:10px;">
                        ${isDemo ? '🎯 What You Will See in the Live Demo' : '🤖 Primary Focus'}
                      </span>
                      <h3 style="margin:0 0 10px;font-size:18px;font-weight:800;color:#0d0d0d;">
                        AI Lead Management System for Real Estate
                      </h3>
                      <p style="margin:0 0 14px;font-size:14px;color:#475569;line-height:1.6;">
                        See how autonomous AI agents respond in <strong>&lt; 60 seconds</strong>, qualify leads in native <strong>Arabic &amp; English</strong>, and book property viewings straight into broker calendars across WhatsApp, Bayut, Property Finder &amp; Meta Ads.
                      </p>
                      <p style="margin:0 0 10px;font-size:13px;font-weight:700;color:#0d0d0d;">
                        ⚡ To tailor your demo walkthrough, simply reply to this email with:
                      </p>
                      <ul style="margin:0 0 18px;padding-left:18px;font-size:13px;color:#475569;line-height:1.6;">
                        ...
                        <li><strong>Active Agents Count</strong> (Team size)</li>
                        <li><strong>Current Lead Channels</strong> (Bayut, Property Finder, WhatsApp, Meta Ads)</li>
                        <li><strong>Current CRM</strong> (HubSpot, Salesforce, Zoho, Bitrix24, etc.)</li>
                      </ul>

                      <table cellpadding="0" cellspacing="0" style="margin-top:8px;">
                        <tr>
                          <td>
                            <a href="https://wa.me/971542968754?text=Hi%20DevMate%2C%20I%20just%20booked%20a%20demo%20for%20the%20AI%20Lead%20Management%20System."
